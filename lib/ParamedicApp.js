...
class ParamedicApp {
    constructor (config, storedCWD, runner) {
        ...
        logger.warn('---------------------------------------------------------');
        logger.warn('1. Create Cordova app with platform and plugin(s) to test');
        logger.warn('   - platform: ' + this.config.getPlatformId());
        logger.warn('   - plugin(s): ' + this.config.getPlugins().join(', '));
        logger.warn('---------------------------------------------------------');
    }
    ...
    buildApp () {
        logger.warn('---------------------------------------------------------');
        logger.warn('3. Build app');
        logger.warn('---------------------------------------------------------');

        return Q()
            .then(() => this.getCommandForBuildingApp())
            .then((command) => {
