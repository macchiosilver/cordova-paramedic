    ...
    run () {
        ...
        return Q().then(() => {
            ...
            .fin((result) => {
                ...
                // If we run --shouldUseSauce fetch and return Sauce details.
                if (this.config.shouldUseSauce()) {
                    logger.normal('Display SauceLabs jobs details:', this.skippedAppium);
                    return this.paramedicSauceLabs.displaySauceDetails.apply(this.paramedicSauceLabs, [this.sauceBuildName]); // eslint-disable-line
                    /*
                    return this.paramedicSauceLabs.displaySauceDetails()
                        .then(() => {
                            if (!this.skippedAppium) { return this.paramedicSauceLabs.displaySauceDetails(null, true); }
                        });
                        */
                }
